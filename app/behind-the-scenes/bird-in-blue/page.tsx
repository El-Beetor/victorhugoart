...
export default function BirdInBlueBehindTheScenes() {
  const { accentColor, darkColors } = useColors();

  // Color palette used in the painting
  const colorPalette = [
    { color: '#87CEEB', name: 'Sky Blue', percentage: 45 },
    { color: '#8B7355', name: 'Brown', percentage: 25 },
    { color: '#F5F5DC', name: 'White', percentage: 15 },
    { color: '#4682B4', name: 'Deep Blue', percentage: 10 },
    { color: '#DAA520', name: 'Yellow', percentage: 3 },
    { color: '#2F4F4F', name: 'Dark Gray', percentage: 2 },
  ];

  // Value breakdown (light to dark)
  const valueBreakdown = [
    { color: '#E8E8E8', name: 'Highlights', percentage: 15 },
    { color: '#B6B6B6', name: 'Lights', percentage: 30 },
    { color: '#787878', name: 'Midtones', percentage: 28 },
    { color: '#404040', name: 'Darks', percentage: 20 },
    { color: '#1C1C1C', name: 'Deep Shadows', percentage: 7 },
  ];
    ...
    <div className="min-h-screen bg-[#fffff7]">
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md" style={{ backgroundColor: '#fffff7DD' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center">
          ...
          <Link href="/" className="px-2 sm:px-4 py-2 rounded-lg font-medium text-sm hover:opacity-70 transition-opacity whitespace-nowrap shrink-0" style={{ color: accentColor }}>
            ← Back to Home
          </Link>
        </div>
      </nav>
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-12">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-4" style={{ color: darkColors[0] || accentColor }}>Bird in Blue</h1>
